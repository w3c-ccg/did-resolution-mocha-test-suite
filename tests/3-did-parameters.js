...
import {checkErrorResolutionResult, checkSuccessfulResolutionResult} from './assertions.js';
  ...
  for(const [name, implementation] of match) {
    ...
    describe(name, function() {
      ...
      it('service parameter MUST be an ASCII string if present', async function() {
        ...
        const urlWithService = addQueryParametersToUrl(url, {service: validService});
        const rv = await fetch(urlWithService);
      ...
      it('serviceType parameter MUST be an ASCII string if present', async function() {
        for(const badServiceType of nonAsciiStrings) {
          const urlWithServiceType = addQueryParametersToUrl(url, {serviceType: badServiceType});
        ...
        const urlWithServiceType = addQueryParametersToUrl(url, {service: validServiceType});
        const rv = await fetch(urlWithServiceType);
        rv.ok.should.be.true;
        rv.status.should.equal(200);
      ...
      it('relativeRef parameter MUST be an ASCII string if present', async function() {
        for(const badRelativeRef of nonAsciiStrings) {
          const urlWithRelativeRef = addQueryParametersToUrl(url, {relativeRef: badRelativeRef});
      ...
      it('relativeRef parameter MUST use percent-encoding for certain characters as specified in RFC3986 Section 2.1.', async function() {
      ...
      it('versionId parameter MUST be an ASCII string if present', async function() {
        for(const badVersionId of nonAsciiStrings) {
          const urlWithVersionId = addQueryParametersToUrl(url, {versionId: badVersionId});
    ...
    it('versionTime parameter MUST be an ASCII string if present', async function() {
      for(const badVersionTime of nonAsciiStrings) {
        const urlWithVersionTime = addQueryParametersToUrl(url, {versionTime: badVersionTime});
    ...
    it('versionTime parameter MUST be a valid XML datetime value if present', async function() {
      for(const badXmlDateTime of invalidXmlDateTimes) {
        const urlWithVersionTime = addQueryParametersToUrl(url, {versionTime: badXmlDateTime});
        const rv = await fetch(urlWithVersionTime);
    ...
    it('versionTime parameter MUST be normalized to UTC 00:00:00 and without sub-second decimal precision. ', async function() {
      ...
      const urlWithVersionTime = addQueryParametersToUrl(url, {versionTime: normalizedDateTime});
      const rv = await fetch(urlWithVersionTime);
